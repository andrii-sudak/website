# Attributions

## Brand Assets & Trademarks

### Signal Logo

* The Signal logo used on this website is an official asset sourced from the Signal brand guidelines page: [https://signal.org/brand/](https://signal.org/brand/).
* Signal® and the Signal logo are registered trademarks of Signal Messenger LLC.
* The logo is used here to refer to the Signal messaging service and/or to link to an official Signal profile, in accordance with fair use principles for trademarks and Signal's published brand guidelines.
* For display purposes on this website, the color of the Signal logo SVG may be adapted to `currentColor` to ensure visual consistency and accessibility across different site themes (e.g., light and dark modes), effectively rendering it as a monochrome version consistent with official black or white variants provided by Signal. This adaptation is solely for thematic integration on this specific website.
* This project/website is not affiliated with, sponsored, or endorsed by Signal Messenger LLC. All rights to the Signal brand and logo are reserved by Signal Messenger LLC.

### Threema Logo

* The Threema logo used on this website is an official asset sourced from the Threema press page: [https://threema.com/en/press](https://threema.com/en/press).
* Threema® and the Threema logo are registered trademarks of Threema GmbH.
* The logo is used here to refer to the Threema messaging service and/or to link to an official Threema profile, in accordance with fair use principles for trademarks and Threema's published brand guidelines.
* For display purposes on this website, the color of the Threema logo SVG may be adapted to `currentColor` to ensure visual consistency and accessibility across different site themes (e.g., light and dark modes), effectively rendering it as a monochrome version consistent with official black or white variants provided by Threema. This adaptation is solely for thematic integration on this specific website.
* This project/website is not affiliated with, sponsored, or endorsed by Threema GmbH. All rights to the Threema brand and logo are reserved by Threema GmbH.

This project uses assets from other sources. We are grateful to their creators for making them available.

### GnuPG Icon

* **Asset File Path in this Repository:** `/assets/icons/gnupg.svg`
* **Description:** This icon is a modified version of the 'lock-wing' icon from The GnuPG Project.  Changes include the addition of a fill attribute
* **Original Icon Source:** `https://git.gnupg.org/cgi-bin/gitweb.cgi?p=gnupg.git;a=blob;f=artwork/icons/lock-wing.svg;hb=a8d0b8d2333ddab703d1e346e06c106eeeedfd53`
* **Original Icon Copyright:** Copyright © The GnuPG Project.
* **License for Original and Modified Version:** This modified icon, as well as the original, is licensed under the **GNU Lesser General Public License v2.1 or later (LGPL-2.1-or-later)**.
* **Source Code for Modified Icon:** The source code for this modified icon is the SVG file located at `/assets/icons/gnupg.svg` in this repository.
* **LGPL v2.1 License Text:**
    * The full text of the LGPL v2.1 is available online at: [https://www.gnu.org/licenses/lgpl-2.1.html](https://www.gnu.org/licenses/lgpl-2.1.html)
    * A copy of `LGPL-2.1.txt` should also be included in this repository (e.g., in a `LICENSES/` directory).
